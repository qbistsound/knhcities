...
public class system 
{
	...
	/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
	private static final String name = "K&H (CITIES)";
	/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
//				for (CSVRecord n : records) { record_size++; n.get(0); }
//			}
//			catch (Exception exception) { exception.printStackTrace(); }
//			//import records
//			try
...
//				long record_index = 0;
//				CSVFormat format = CSVFormat.DEFAULT;
//				system.console.write(String.format("Importing [%d] records from (%s)", record_size, path));
//				Reader reader = new StringReader(new String(Files.readAllBytes(Paths.get(path))));
//				Iterable<CSVRecord> records = format.parse(reader);
...
//					String name = n.get(1);
//					String urld = n.get(2);
//					system.console.percent(record_index, record_size);
//					record_index++;
//					
//					if (!system.isnum(id)) { continue; }
//					//image
//					String data = "*";
//					try
//					{
...
//					command.setInt(1, Integer.valueOf(id).intValue());
...
//					exception.printStackTrace();
	...
	class _image extends Thread
	{
		...
		/*--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
		public _image(int id, String url, String name)
		{
			this.id = id;
			this.url = url;
			this.name = name;
